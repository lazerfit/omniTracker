...
describe('getDb', () => {
  ...
  it('calling getDb() twice returns the same singleton instance', () => {
    const a = getDb();
    const b = getDb();
    expect(a).toBe(b);
  });

  it('exchange_keys table exists after init', () => {
    const db = getDb();
    const row = db
  ...
  it('portfolio_snapshots table exists after init', () => {
    const db = getDb();
    const row = db
      .query<{ name: string }, [string]>(
        "SELECT name FROM sqlite_master WHERE type='table' AND name=?",
  ...
  it('can insert and query a row in exchange_keys', () => {
    const db = getDb();
    db.run(
  ...
  it('can insert and query a row in portfolio_snapshots', () => {
    const db = getDb();
    db.run(
